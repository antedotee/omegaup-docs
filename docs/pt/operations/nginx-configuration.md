...
omegaUp usa Nginx como servidor web e proxy reverso. Este guia aborda a configuração para ambientes de desenvolvimento e produção.
...
A configuração de desenvolvimento do Nginx está localizada em `stuff/docker/etc/nginx/nginx.conf`:
...
O arquivo `nginx.rewrites` lida com URLs limpos:
...
```
## Balanceamento de carga (vários servidores)
